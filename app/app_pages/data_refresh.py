...
with col2:
    st.metric(
        "RSS Feeds", len(aws_blog_feeds), help="Number of RSS feeds being monitored"
    )
with col3:
    st.metric(
        "Categories",
...
with st.expander("View RSS Feed Details", expanded=True):
    feed_df = pd.DataFrame(
        [
            {"Category": category.capitalize(), "Feed URL": url, "Status": "Active ✅"}
    ...
    st.dataframe(
        ...
        column_config={
            "Category": st.column_config.TextColumn(
                "Category", help="Blog category", width="medium"
            ),
            "Feed URL": st.column_config.LinkColumn(
                "Feed URL", help="Click to open RSS feed", width="large"
            ),
            "Status": st.column_config.TextColumn(
                "Status", help="Current feed status", width="small"
            ),
        },
    )
...
with refresh_col2:
    if st.button("Refresh Now", type="primary", use_container_width=True):
        ...
        add_rss_feed_refresh(
            lancedb_manager, blog_feeds=aws_blog_feeds, records_metric=records_metric
        )
